const products = [
  {
    'id': '1',
    'product_name': 'Hachis Parmentier',
    'brand': 'Picard',
    'quantity': '1kg',
    'packaging': [
      'carton',
      'surgelé'
    ],
    'categories': [
      'Plats préparés',
      'Plats à base de viande',
      'Plats à base de viande bovine',
      'Hachis parmentier',
      'Gamme famille picard',
      'Hachis parmentier surgelé'
    ],
    'labels': [
      'Viande Française',
      'Viande Bovine Française'
    ],
    'country': 'France',
    'ingredients': 'Pomme de terre 42,9%, viande bovine 25,1% (origine : France*), eau, crème fraiche légère liquide (lait) 3,9%, chapelures 2,9% [farine de blé (gluten), sel, levure], oignon, fécule de pomme de terre, emmental français râpé 0,7% (contient : lait), purée de tomate double concentrée, beurre (lait), carotte, sel, ail, poireau, persil, oignon en poudre, thym, poivre, plantes aromatiques (persil, laurier, thym), curcuma, muscade.',
    'allergenes': [
      'Gluten',
      'lait'
    ],
    'nova': 3,
    'nutriscore': 'B',
    'nutritional_value': {
      'energy': '108', // kcal
      'lipides': '4.7',// g
      'saturated_fat': '2.5',// g
      'glucides': '9.8',// g
      'sugar': '0.4',// g
      'fibres_alimentaires': '0.8',// g
      'proteines': '6.5',// g
      'salt': '0.5',// g
      'sodium': '0.197' // g
    },
    'image': 'https://static.openfoodfacts.org/images/products/327/016/022/7495/front_fr.23.400.jpg',
    'keywords': [
      'hachis',
      'parmentier',
      'surgelé',
      'boeuf',
    ],
  },

  // Produit 2

  {
    'id': '2',
    'product_name': 'Emietté de sardine - douceur des bastides',
    'brand': 'La Belle-Iloise',
    'quantity': '80g',
    'packaging': [
      'conserve',
    ],
    'categories': [
      'Conserves',
      'Produits de la mer',
      'Poissons',
      'Poissons en conserve',
      'Sardines',
      'Sardines en conserve',
      'Sardines à l\'huile',
      'Sardines à l\'huile d\'olive'
    ],
    'labels': [],
    'country': 'France',
    'ingredients': 'Filets de sardines (39,6%), huile d\'olive vierge extra, fenouil (14,9%), oignons (9,3%), jus de citron, tomates (2,9%), tomates mi-séchées (2,8%), oignons rissolés (2,4%), fécule de manioc, ail, plantes aromatiques (0,9%), sel, badiane, épices.',
    'allergenes': [
      'Poisson',
    ],
    'nova': 3,
    'nutriscore': 'B',
    'nutritional_value': {
      'energy': '234',
      'lipides': '18.9',
      'saturated_fat': '3.18',
      'glucides': '3.2',
      'sugar': '0.4',
      'fibres_alimentaires': '0',
      'proteines': '12.3',
      'salt': '0.5',
      'sodium': '0.197'
    },
    'image': 'https://static.openfoodfacts.org/images/products/366/008/813/8643/front_fr.14.400.jpg',
    'keywords': [
      'poisson',
      'sardine',
      'conserve',
    ],
  },

  // Produit 3

  {
    'id': '3',
    'product_name': 'Emiette de sardine citron vert',
    'brand': 'La Belle-Iloise',
    'quantity': '80g',
    'packaging': [
      'conserve',
    ],
    'categories': [
      'Plats préparés',
      'Plats à base de poisson'
    ],
    'labels': [],
    'country': 'France',
    'ingredients': 'Filets de sardines (49,2 %), huile d\'olive vierge extra, oignons rissolés, jus de citron (5,9 %), olives vertes (5,8 %), amandes grillées (2,5 %), plantes aromatiques, citron (1,7 %), fécule de manioc, sel, sucre, épices.',
    'allergenes': [
      'poisson',
      'fruits à coque'
    ],
    'nova': 3,
    'nutriscore': 'C',
    'nutritional_value': {
      'energy': '276',
      'lipides': '22.8',
      'saturated_fat': '4.17',
      'glucides': '4.6',
      'sugar': '1.2',
      'fibres_alimentaires': '0',
      'proteines': '12.4',
      'salt': '0.3',
      'sodium': '0.118'
    },
    'image': 'https://static.openfoodfacts.org/images/products/366/008/813/6748/front_fr.10.400.jpg',
    'keywords': [
      'poisson',
      'sardine',
      'conserve',
      'citron',
      'amandes'
    ],
  },

  // Produit 4

  {
    'id': '4',
    'product_name': 'Lait Demi-Écrémé',
    'brand': 'Leader Price',
    'quantity': '1L',
    'packaging': [
      'bouteille',
      'bouchon',
      'plastique'
    ],
    'categories': [
      'Produits laitiers',
      'Laits',
      'Laits homogénéisés',
      'Laits UHT',
      'Laits demi-écrémés'
    ],
    'labels': [
      'Point Vert',
      'Garantie IPLC',
      'Eco-Emballages',
      'Elaboré en France'
    ],
    'country': 'France',
    'ingredients': 'Lait demi-écrémé stérilisé UHT.',
    'allergenes': [
      'lait'
    ],
    'nova': 1,
    'nutriscore': 'B',
    'nutritional_value': {
      'energy': '192',
      'lipides': '1.55',
      'saturated_fat': '1',
      'glucides': '4.8',
      'sugar': '4.8',
      'fibres_alimentaires': '0',
      'proteines': '3.2',
      'salt': '0.1',
      'sodium': '0.0394'
    },
    'image': 'https://static.openfoodfacts.org/images/products/326/385/509/3697/front_fr.7.400.jpg',
    'keywords': [
      'lait',
      'vache',
      'demi',
      'écrémé',
      'demi-écrémé'
    ],
  },

  // Produit 5

  {
    'id': '5',
    'product_name': 'Mini Bubblizz',
    'brand': 'Lutti ',
    'quantity': '130g',
    'packaging': [
      'sachet',
      'plastique'
    ],
    'categories': [
      'snacks sucrés',
      'confiserie',
      'bonbons',
      'bonbons gélifiés'
    ],
    'labels': [
      'Peu ou pas de sucre',
      'Peu de sucre',
      'Allégé en sucre',
      'Moins 40% de sucre',
      'Sans gluten',
      'Colorants naturels',
      'Sans édulcorants'
    ],
    'country': 'France',
    'ingredients': 'Sirop de glucose, sucre, fibre soluble de maïs, gélatine, acidifiants (acide citrique, acide malique), correcteurs d\'acidité (carbonate de calcium, malades de sodium), arôme, denrées alimentaires colorantes (concentré de spiruline et pomme), colorant (anthocyanes)',
    'allergenes': [],
    'additifs': [
      'E428 - Gélatine',
      'E330 - Acide citrique',
      'E296 - Acide malique',
      'E170 - Carbonate de calcium',
      'E350 - Malate de sodium',
      'E163 - Anthocyanes'
    ],
    'nova': 4,
    'nutriscore': 'C',
    'nutritional_value': {
      'energy': '315',
      'lipides': '0',
      'saturated_fat': '0',
      'glucides': '66',
      'sugar': '35',
      'fibres_alimentaires': '10',
      'proteines': '6',
      'salt': '0.3',
      'sodium': '0.118'
    },
    'image': 'https://static.openfoodfacts.org/images/products/311/674/003/4728/front_fr.4.400.jpg',
    'keywords': [
      'lutti',
      'bonbons',
    ],
  },

  // Produit 6

  {
    'id': '6',
    'product_name': 'Coca-Cola',
    'brand': 'Coca-Cola',
    'quantity': '50cl',
    'packaging': [
      'bouteille',
      'plastique'
    ],
    'categories': [
      'Boissons',
      'Boissons gazeuses',
      'Sodas',
      'Sodas au cola',
      'Boissons sucrées'
    ],
    'labels': [],
    'country': 'France',
    'ingredients': 'Eau pétillante, sucre, colorant E150d, acidifiant E338, arômes naturels (extraits végétaux), dont caféine.',
    'additifs': [
      'E150d - Caramel au sulfite d\'ammonium',
      'E338 - Acide orthophosphorique'
    ],
    'nova': 4,
    'nutriscore': 'E',
    'nutritional_value': {
      'energy': '176',
      'lipides': '0',
      'saturated_fat': '0',
      'glucides': '10.6',
      'sugar': '10.6',
      'fibres_alimentaires': '0',
      'proteines': '0',
      'salt': '0',
      'sodium': '0'
    },
    'image': 'https://static.openfoodfacts.org/images/products/54491472/front_fr.81.400.jpg',
    'keywords': [
      'coca',
      'cola',
      'boisson',
      'gaseuze'
    ],
  },

  // Produit 7

  {
    'id': '7',
    'product_name': 'Chips saveur poulet rôti',
    'brand': 'Chipso', // Illisible
    'quantity': '150g',
    'packaging': [
      'sachet',
    ],
    'categories': [
      'chips',
    ],
    'labels': [
      'Éco-Emballages'
    ],
    'country': 'France',
    'nova': 3,
    'nutriscore': 'D',
    'nutritional_value': {
      'energy': '552',
      'lipides': '35',
      'saturated_fat': '3.2',
      'glucides': '51',
      'sugar': '1.5',
      'fibres_alimentaires': '0',
      'proteines': '5.6',
      'salt': '1.5',
      'sodium': '0.591'
    },
    'image': 'https://static.openfoodfacts.org/images/products/26017082/front_fr.6.400.jpg',
    'keywords': [
      'chips',
      'poulet',
      'roti',
    ],
  },

  // Produit 8

  {
    'id': '8',
    'product_name': 'Billes De Cereales Granola Chocolat',
    'brand': 'Made Good',
    'quantity': '24g',
    'packaging': [
      'sachet',
      'plastique'
    ],
    'categories': [
      'snacks sucrés'
    ],
    'labels': [
      'Bio', 'Végétarien', 'Végétalien', 'Sans arachide', 'CA-BIO-154', 'Union Orthodoxe'
    ],
    'country': 'France',
    'ingredients': 'Flocons d\'avoine pure (41 %), pépites de chocolat (sucre de canne, liqueur de cacao, beurre de cacao, extrait de huile de tournesol, sucre de betterave, nectar d\'agave, pomme, riz brun complet croustillant, inuline d\'agave, farine de tapioca, poudre de légumes (épinards, brocolis, carottes tomates, betteraves) (0,4%), poudre de champignon shiitake, arôme chocolat, arôme vanille.',
    'allergenes': [
      'gluten',
    ],
    'nova': 4,
    'nutriscore': 'D',
    'nutritional_value': {
      'energy': '409',
      'lipides': '19',
      'saturated_fat': '9',
      'glucides': '58',
      'sugar': '22',
      'fibres_alimentaires': '7',
      'proteines': '7',
      'salt': '0.03',
      'sodium': '0.0118'
    },
    'image': 'https://static.openfoodfacts.org/images/products/068/745/622/2011/front_fr.3.400.jpg',
    'keywords': [
      'bio',
      'muesli',
      'chocolat',
    ],
  },

  // Produit 9

  {
    'id': '9',
    'product_name': 'Nutella',
    'brand': 'Ferrero',
    'quantity': '600g',
    'packaging': [
      'Verre',
      'Bocal',
      'Plastique'
    ],
    'categories': [
      'Petit-déjeuners',
      'Produits à tartiner',
      'Produits à tartiner sucrés',
      'Pâtes à tartiner, Pâtes à tartiner au chocolat',
      'Pâtes à tartiner aux noisettes',
      'Pâtes à tartiner aux noisettes et au cacao'
    ],
    'labels': [
      'Sans gluten',
      'Point Vert'
    ],
    'country': 'France',
    'ingredients': 'Sucre, huile de palme, NOISETTES 13%, cacao maigre LAIT écrémé en poudre 6,6%, LACTOSERUM en poudre, émulsifiants : lécithines (soja), vanilline.',
    'allergenes': [
      'Lait',
      'Fruits à coque',
      'Soja',
    ],
    'additifs': [
      'E322 - Lécithines'
    ],
    'nova': 4,
    'nutriscore': 'E',
    'nutritional_value': {
      'energy': '546',
      'lipides': '31.6',
      'saturated_fat': '11',
      'glucides': '57.6',
      'sugar': '56.8',
      'fibres_alimentaires': '0',
      'proteines': '6',
      'salt': '0.114',
      'sodium': '0.0449'
    },
    'image': 'https://static.openfoodfacts.org/images/products/301/762/040/6003/front_fr.83.400.jpg',
    'keywords': [
      'nutella',
      'pâte à tartiner',
      'chocolat',
      'noisette',
      'déjeuner'
    ],
  },

  // Produit 10

  {
    'id': '10',
    'product_name': 'Croc\' tou pommes et fraises',
    'brand': 'Il était un fruit',
    'quantity': '60g',
    'packaging': [
      'sachet',
      'boite carton'
    ],
    'categories': [
      'snacks sucrés',
      'produits déshydratés'
    ],
    'labels': [],
    'country': 'France',
    'ingredients': 'Pomme (85%), Fraise (15%)',
    'nova': 3,
    'nutriscore': 'B',
    'nutritional_value': {
      'energy': '315',
      'lipides': '0.667',
      'saturated_fat': '0.667',
      'glucides': '69.3',
      'sugar': '54.7',
      'fibres_alimentaires': '12',
      'proteines': '3.33',
      'salt': '0.0667',
      'sodium': '0.0262'
    },
    'image': 'https://static.openfoodfacts.org/images/products/377/000/123/0047/front_fr.24.400.jpg',
    'keywords': [
      'fruits',
      'déshydraté',
      'pomme',
      'fraise'
    ],
  },

  // Produit 11

  {
    'id': '11',
    'product_name': 'Jambon Cuit Supérieur Bio avec Couenne',
    'brand': 'Bonval',
    'quantity': '180g',
    'packaging': [
      'frais',
      'plastique'
    ],
    'categories': [
      'Viandes',
      'Charcuteries',
      'Jambons',
      'Jambons blancs'
    ],
    'labels': [
      'Bio',
      'Bio européen',
      'Viande Française',
      'FR-BIO-10',
      'Porc Français',
      'Point Vert',
      'Fabriqué en France',
      'AB Agriculture Biologique',
      'Fabrication artisanale',
      'Agriculture France'
    ],
    'country': 'France',
    'ingredients': 'Jambon de porc (origine France) avec couenne*, saumure*, eau, sel, dextrose de maïs*, antioxydant : ascorbate de sodium, plantes aromatiques*, épices*, conservateur : nitrite de sodium). *Ingrédients issus de l\'agriculture biologique.',
    'additifs': [
      'E301 - Ascorbate de sodium',
      'E250 - Nitrite de sodium'
    ],
    'nova': 4,
    'nutriscore': 'C',
    'nutritional_value': {
      'energy': '114',
      'lipides': '3',
      'saturated_fat': '0.9',
      'glucides': '0.6',
      'sugar': '0.4',
      'fibres_alimentaires': '0',
      'proteines': '21',
      'salt': '1.8',
      'sodium': '0.709'
    },
    'image': 'https://static.openfoodfacts.org/images/products/376/009/953/4141/front_fr.5.400.jpg',
    'keywords': [
      'bio',
      'porc',
      'jambon',
      'tranches'
    ],
  },

  // Produit 12

  {
    'id': '12',
    'product_name': 'Salade mezze filet de poulet',
    'brand': 'Mix',
    'quantity': '320g',
    'packaging': [
      'carton'
    ],
    'categories': [
      'Plats préparés',
      'Salades composées'
    ],
    'labels': [
      'Fourchette incluse',
      'Même saveur',
      'Nouveau look',
      'Sauce incluse'
    ],
    'country': 'UE',
    'ingredients': '106,5 g de Penne cuisinées (33,3%) : Pâtes alimentaires aux oeufs cuites 20.7% (eau, semoule de blé dur, oeuf), tomates 10.6%, basilic, huile d’olive vierge extra, sel, poivre blanc. 70 g d’Ingrédients Savoureux (21,9%) : Emincés de filet de poulet traités en salaison cuits rôtis 12.4% (filet de poulet 11,9% (origine: UE), dextrose, proteines de pois, sel, arômes naturels, ferments), tomates cerise 6.2%, copeaux de fromage italien 3.1% (lait de vache cru, sel, présure, conservateur : lysozyme (oeuf)), basilic. 50 g de Salades mélangées (15,6%) Laitue, chicorée et roquette. 50 g de Mélange de tomates fraîches et tomates mi-séchées marinées, assaisonné (15,6%) : Tomates fraîches 12%, sauce [double concentré de tomates 0.5%, huile de colza, eau, sel, vinaigre d’alcool, moutarde de Dijon (eau, graines de moutarde, vinaigre d’alcool, sel), amidon modifié de pomme de terre, épaississant : gomme xanthane, jus de citron concentré, poivre blanc], tomates mi-séchées marinées 0.7% (tomates, huile de tournesol, sel, plantes aromatiques), huile d’olive vierge extra, plantes aromatiques, sucre, acidifiant : acide citrique, conservateur : sorbate de potassium. 30 g de Sauce au Parmigiano Reggiano (9,4%):Eau, huile de colza, huile d’olive vierge extra, Parmigiano Reggiano 0.4%, moutarde de Dijon (eau, graines de moutarde, vinaigre, sel, antioxydant : disulfite de potassium, acidifiant : acide citrique), jus concentré de citron, sel, jaune d’oeuf, ail, amidon modifié de pomme de terre, basilic, acidifiant : acide lactique, épaississant : gomme xanthane, conservateur : sorbate de potassium. 13,5 g de Petits Blinis nature (4,2%) : Eau, farine de blé, huile de tournesol, lactose, proteines de lait, lait écrémé en poudre, lactosérum en poudre, oeuf entier et blanc d’oeuf déshydratés, poudre à lever : carbonates de sodium et diphosphates, sel, antioxydant : acide ascorbique.',
    'allergenes': [
      'Œufs',
      'Gluten',
      'Lait',
      'Moutarde',
      'Parmigiano Reggiano'
    ],
    'additifs': [
      'E1105 - Lysozyme',
      'E14XX - Amidons modifiés',
      'E415 - Gomme xanthane',
      'E330 - Acide citrique',
      'E202 - Sorbate de potassium',
      'E224 - Disulfite de potassium',
      'E270 - Acide lactique',
      'E500 - Carbonates de sodium',
      'E450 - Sels métalliques de diphosphates',
      'E300 - Acide ascorbique'
    ],
    'nova': 4,
    'nutriscore': 'B',
    'nutritional_value': {
      'energy': '117',
      'lipides': '6.2',
      'saturated_fat': '1.1',
      'glucides': '8.3',
      'sugar': '2.3',
      'fibres_alimentaires': '1.8',
      'proteines': '6.1',
      'salt': '0.8',
      'sodium': '0.315'
    },
    'image': 'https://static.openfoodfacts.org/images/products/370/000/925/7906/front_fr.17.400.jpg',
    'keywords': [
      'salade',
      'composée',
      'plat',
      'préparé',
      'oeuf',
      'pâtes'
    ],
  },

  // Produit 13

  {
    'id': '13',
    'product_name': 'Badoit',
    'brand': 'Badoit',
    'quantity': '50 cl',
    'packaging': [
      'bouteille',
      'plastique'
    ],
    'categories': [
      'Boissons',
      'Eaux',
      'Boissons gazeuses',
      'Eaux de sources',
      'Eaux minérales',
      'Eaux gazeuses',
      'Eaux minérales naturelles',
      'Boissons non sucrées'
    ],
    'labels': [],
    'country': 'France',
    'ingredients': 'Eau',
    'allergenes': [],
    'nova': 1,
    'nutriscore': '',
    'nutritional_value': {
      'energy': '0',
      'lipides': '0',
      'saturated_fat': '0',
      'glucides': '0',
      'sugar': '0',
      'fibres_alimentaires': '0',
      'proteines': '0',
      'salt': '0.4',
      'sodium': '0.157'
    },
    'image': 'https://static.openfoodfacts.org/images/products/306/832/009/9613/front_fr.3.400.jpg',
    'keywords': [
      'eau',
      'gazeuse',
      'boisson',
      'naturel'
    ],
  },

  // Produit 14

  {
    'id': '14',
    'product_name': 'Réveil fruité',
    'brand': 'Tropicana',
    'quantity': '1.5L',
    'packaging': [
      'bouteille',
      'plastique',
      'bouteille plastique'
    ],
    'categories': [
      'Aliments et boissons à base de végétaux',
      'Boissons',
      'Boissons à base de végétaux',
      'Boissons aux fruits',
      'Jus et nectars',
      'Jus et nectars de fruits',
      'Jus de fruits',
      'Jus multifruits',
      'Jus de fruits 100% pur jus'
    ],
    'labels': [
      'Peu ou pas de sucre',
      'Sans sucre ajouté'
    ],
    'country': 'France',
    'ingredients': 'Jus de mandarine, d\'orange, de pomme et de raisin blanc',
    'additifs': [],
    'allergenes': [
      'orange'
    ],
    'nova': 1,
    'nutriscore': 'D',
    'nutritional_value': {
      'energy': '51',
      'lipides': '0.32',
      'saturated_fat': '0',
      'glucides': '11',
      'sugar': '10.9',
      'fibres_alimentaires': '0.8',
      'proteines': '0.6',
      'salt': '0',
      'sodium': '0'
    },
    'image': 'https://static.openfoodfacts.org/images/products/541/018/801/3085/front_fr.75.400.jpg',
    'keywords': [
      'jus',
      'fruit',
      'boisson',
      'matin'
    ],
  },

  // Produit 15

  {
    'id': '15',
    'product_name': 'Mielnuts',
    'brand': 'Auchan',
    'quantity': '375g',
    'packaging': [
      'carton',
      'boîte',
    ],
    'categories': [
      'Aliments et boissons à base de végétaux',
      'Aliments d\'origine végétale',
      'Petit-déjeuners',
      'Céréales et pommes de terre',
      'Céréales et dérivés',
      'Céréales pour petit-déjeuner',
      'Céréales au miel'
    ],
    'labels': [],
    'country': 'France',
    'ingredients': 'Semoule de maïs 78 %, sucre, miel 3,5 %, sel, colorant : caramel (E150c), vitamines : niacine (PP), acide pantothénique (B5), riboflavine (B2), thiamine (B1), vitamine B6, acide folique (B9), vitamine B12, fer.',
    'additifs': [
      'E150c - Caramel ammoniacal'
    ],
    'allergenes': [],
    'nova': 4,
    'nutriscore': 'C',
    'nutritional_value': {
      'energy': '381',
      'lipides': '0.9',
      'saturated_fat': '0.2',
      'glucides': '86',
      'sugar': '27',
      'fibres_alimentaires': '2.1',
      'proteines': '6.2',
      'salt': '0.5',
      'sodium': '0.197'
    },
    'image': 'https://static.openfoodfacts.org/images/products/325/456/004/0489/front_fr.16.400.jpg',
    'keywords': [
      'céréales',
      'miel',
      'auchan',
    ],
  },

  // Produit 16

  {
    'id': '16',
    'product_name': 'Miel Pops',
    'brand': 'Kellogg\'s ',
    'quantity': '375g',
    'packaging': [
      'carton',
      'sachet plastique',
    ],
    'categories': [
      'Aliments et boissons à base de végétaux',
      'Aliments d\'origine végétale',
      'Petit-déjeuners',
      'Céréales et pommes de terre',
      'Céréales et dérivés',
      'Céréales pour petit-déjeuner',
      'Céréales au miel'
    ],
    'labels': [],
    'country': 'France',
    'ingredients': 'Farine de maïs, sucre, sirop de glucose, miel (5%), sel, extrait de malt d\'orge, colorants (caramel E150c, carotènes), vitamines (vitamine PP, vitamine B6, vitamine B2, vitamine B1, vitamine B9, vitamine D, vitamine B12), phosphate tricalcique, fer',
    'additifs': [
      'E150c - Caramel ammoniacal',
      'E160 - Caroténoïdes'
    ],
    'allergenes': [],
    'nova': 4,
    'nutriscore': 'D',
    'nutritional_value': {
      'energy': '389',
      'lipides': '1',
      'saturated_fat': '0.2',
      'glucides': '88',
      'sugar': '28',
      'fibres_alimentaires': '1',
      'proteines': '5',
      'salt': '0.8382',
      'sodium': '0.33'
    },
    'image': 'https://static.openfoodfacts.org/images/products/315/947/000/1011/front_fr.6.400.jpg',
    'keywords': [
      'céréales',
      'miel',
      'kellogs',
    ],
  },

  // Produit 17

  {
    'id': '17',
    'product_name': 'Haricots Blancs à la Tomate',
    'brand': 'Notre Jardin',
    'quantity': '800g',
    'packaging': [
      'conserve',
      'metal',
    ],
    'categories': [
      'Aliments et boissons à base de végétaux',
      'Aliments d\'origine végétale',
      'Légumineuses et dérivés',
      'Conserves',
      'Aliments à base de plantes en conserve',
      'Plats préparés',
      'Légumineuses en conserve',
      'Légumes préparés',
      'Haricots en conserve',
      'Haricots à la tomate',
      'Haricots blancs en conserve'
    ],
    'labels': [
      'Point Vert'
    ],
    'country': 'France',
    'ingredients': 'Haricots blancs trempés, eau, double concentré de tomates 2,8%, sel, amidon modifié de maïs, oignon en poudre, arôme naturel.',
    'additifs': [],
    'allergenes': [],
    'nova': 4,
    'nutriscore': 'A',
    'nutritional_value': {
      'energy': '74',
      'lipides': '0.1',
      'saturated_fat': '0',
      'glucides': '9.9',
      'sugar': '0.3',
      'fibres_alimentaires': '6',
      'proteines': '6',
      'salt': '0.91',
      'sodium': '0.358'
    },
    'image': 'https://static.openfoodfacts.org/images/products/356/470/073/8009/front_fr.7.400.jpg',
    'keywords': [
      'conserve',
      'légumes',
      'haricots',
      'légumineuses',
    ],
  },

  // Produit 18

  {
    'id': '18',
    'product_name': 'Viande hachée pur boeuf',
    'brand': 'Férial',
    'quantity': '350g',
    'packaging': [
      'plastique',
      'barquette',
      'frais'
    ],
    'categories': [
      'Viandes',
      'Bœuf',
      'Viandes fraîches'
    ],
    'labels': [
      'Viande Française',
      '100% muscle',
      'Viande Bovine Française'
    ],
    'country': 'France',
    'ingredients': '100 % pure viande bovine.',
    'additifs': [],
    'allergenes': [],
    'nova': 1,
    'nutriscore': 'A',
    'nutritional_value': {
      'energy': '145',
      'lipides': '5',
      'saturated_fat': '0',
      'glucides': '0',
      'sugar': '0',
      'fibres_alimentaires': '0',
      'proteines': '25',
      'salt': '0',
      'sodium': '0'
    },
    'image': 'https://static.openfoodfacts.org/images/products/366/111/209/2108/front_fr.15.400.jpg',
    'keywords': [
      'viande',
      'boeuf',
    ],
  },

  // Produit 19

  {
    'id': '19',
    'product_name': 'Balisto miel amandes',
    'brand': 'Balisto',
    'quantity': '167g',
    'packaging': [
      'sachet',
      'plastique'
    ],
    'categories': [
      'Snacks sucrés',
      'Confiseries',
      'Confiseries chocolatées',
      'Barres',
      'Barres chocolatées'
    ],
    'labels': [
      'Agriculture durable',
      'UTZ Certified',
      'Cacao certifié UTZ'
    ],
    'country': 'Allemagne',
    'ingredients': 'Sucre, céréales complètes (19%) (flocons d\'avoine complet, farine de blé complète, farine d\'orge complète), huile de tournesol, farine de blé, beurre de cacao, amandes (5.0%), pâte de cacao, lait entier en poudre, lait écrémé en poudre, matière grasse de karité, lactose, beurre concentré (lait), petit-lait en poudre, émulsifiants (lécithine de soja, mono- et diglycérides d\'acides gras, stéaroyl-2-lactylate de sodium), matière grasse de palme, sel, poudre à lever (carbonate de sodium), huile de noix de coco, miel, arômes naturels, extrait naturel de vanille.',
    'additifs': [
      'E322i - Lécithine',
      'E471 - Mono- et diglycérides d\'acides gras alimentaires',
      'E481 - Stéaroyl-2-lactylate de sodium',
      'E500i - Carbonate de sodium'
    ],
    'allergenes': [
      'Gluten',
      'Lait',
      'Fruits à coque',
      'Soja'
    ],
    'nova': 4,
    'nutriscore': 'E',
    'nutritional_value': {
      'energy': '518',
      'lipides': '27.7',
      'saturated_fat': '11.2',
      'glucides': '58.5',
      'sugar': '42.1',
      'fibres_alimentaires': '0',
      'proteines': '6.8',
      'salt': '0.7',
      'sodium': '0.276'
    },
    'image': 'https://static.openfoodfacts.org/images/products/500/015/941/9123/front_fr.19.400.jpg',
    'keywords': [
      'balisto',
      'miel',
      'amandes',
      'gateaux',
      'snacks'
    ],
  },

  // Produit 20

  {
    'id': '20',
    'product_name': 'Jambon supérieur (-25 % de sel)',
    'brand': 'Saint Alby',
    'quantity': '180g',
    'packaging': [
      'frais',
      'plastique',
      'barquette'
    ],
    'categories': [
      'Viandes',
      'Charcuteries',
      'Jambons',
      'Produits à teneur réduite en sel',
      'Charcuteries à teneur réduite en sel',
      'Jambons blancs',
      'Jambons blancs à teneur réduite en sel',
    ],
    'labels': [
      'Peu ou pas de sel',
      'Peu de sel',
      'Allégé en sel',
      'Viande Française',
      'Moins 25% de sel',
      'Porc Français'
    ],
    'country': 'France',
    'ingredients': 'Jambon de porc, sel, sirop de glucose, arômes, chlorure de potassium, antioxydant : érythorbate de sodium, conservateur : nitrite de sodium.',
    'additifs': [
      'E316 - Erythorbate de sodium',
      'E250 - Nitrite de sodium',
    ],
    'allergenes': [],
    'nova': 4,
    'nutriscore': 'C',
    'nutritional_value': {
      'energy': '113',
      'lipides': '3',
      'saturated_fat': '1.2',
      'glucides': '0.5',
      'sugar': '0',
      'fibres_alimentaires': '0',
      'proteines': '21',
      'salt': '1.5',
      'sodium': '0.591'
    },
    'image': 'https://static.openfoodfacts.org/images/products/20458478/front_fr.12.400.jpg',
    'keywords': [
      'jambon',
      'proc',
      'tranches'
    ],
  },

  // Produit 21

  {
    'id': '21',
    'product_name': 'Petits pois carottes à l\'étuvé',
    'brand': 'Auchan',
    'quantity': '800g',
    'packaging': [
      'boite metal',
      'conserve'
    ],
    'categories': [
      'Petits pois et carottes en conserve'
    ],
    'labels': [],
    'country': 'France',
    'ingredients': 'Légumes (carottes 32%, petits pois 30%), eau, sucre, sel, arôme naturel d\'herbes.',
    'additifs': [],
    'allergenes': [],
    'nova': 3,
    'nutriscore': 'A',
    'nutritional_value': {
      'energy': '50',
      'lipides': '0.5',
      'saturated_fat': '0.1',
      'glucides': '7',
      'sugar': '3.7',
      'fibres_alimentaires': '4.1',
      'proteines': '2.8',
      'salt': '0.53',
      'sodium': '0.209'
    },
    'image': 'https://static.openfoodfacts.org/images/products/359/671/045/6819/front_fr.4.400.jpg',
    'keywords': [
      'petits pois',
      'carottes',
      'légumes',
      'conserve'
    ],
  },

  // Produit 22

  {
    'id': '22',
    'product_name': 'Bûche de Chèvre Sainte Maure',
    'brand': 'Auchan',
    'quantity': '200g',
    'packaging': [
      'papier'
    ],
    'categories': [
      'Dairies',
      'Fermented foods',
      'Fermented milk products',
      'Cheeses',
      'Pasteurized cheeses',
      'Goat cheeses'
    ],
    'labels': [],
    'country': 'France',
    'ingredients': 'Lait de chèvre pasteurisé, sel',
    'additifs': [],
    'allergenes': [],
    'nutriscore': 'D',
    'nutritional_value': {
      'energy': '293',
      'lipides': '23',
      'saturated_fat': '16.1',
      'glucides': '1',
      'sugar': '1',
      'fibres_alimentaires': '0',
      'proteines': '20',
      'salt': '1.5',
      'sodium': '0.591'
    },
    'image': 'https://static.openfoodfacts.org/images/products/325/456/032/1434/front_fr.5.400.jpg',
    'keywords': [
      'chèvre',
      'fromage',
      'lait',
    ],
  },

  // Produit 23

  {
    'id': '23',
    'product_name': 'THÉ NOIR parfum MÛRE MYRTILLE ',
    'brand': 'May Tea',
    'quantity': '1L',
    'packaging': [
      'Bouteille',
      'Plastique',
      'bouteille et bouchon plastique à recycler'
    ],
    'categories': [
      'boissons',
      'thé glacé',
      'boissons sucrées',
      'Thés glacés saveur mûre'
    ],
    'labels': [
      'No colorings',
      'No preservatives',
      'Green Dot',
      'New',
      'No artificial flavors',
      'Without sweeteners',
      'Tidy man'
    ],
    'country': 'France',
    'ingredients': 'Infusion de thé noir 94% (eau, infusion intense de thé noir), sucre, jus de mûre à base de concentré 1%, acidifiants : acide malique et acide citrique, arôme naturel de myrtille avec autres arômes naturels, correcteur d\'acidité : citrate de sodium, antioxydant : acide ascorbique.',
    'additifs': [
      'E296 - Malic acid',
      'E330 - Citric acid',
      'E331 - Sodium citrates',
      'E300 - Ascorbic acid',
    ],
    'allergenes': [],
    'nutriscore': 'D',
    'nutritional_value': {
      'energy': '18',
      'lipides': '0',
      'saturated_fat': '0',
      'glucides': '4.3',
      'sugar': '4.3',
      'fibres_alimentaires': '0',
      'proteines': '0',
      'salt': '0',
      'sodium': '0'
    },
    'image': 'https://static.openfoodfacts.org/images/products/312/448/018/9110/front_fr.56.400.jpg',
    'keywords': [
      'thé',
      'glacé',
      'boisson',
    ],
  },

  // Produit 24

  {
    'id': '24',
    'product_name': 'Chips saveur chèvre chaud & herbes',
    'brand': 'Vico',
    'quantity': '120g',
    'packaging': [
      'sachet',
      'Plastique'
    ],
    'categories': [
      'Plant-based foods and beverages',
      'Plant-based foods',
      'Cereals and potatoes',
      'Salty snacks',
      'Appetizers',
      'Chips and fries',
      'Crisps',
      'Potato crisps',
      'Flavoured potato crisps',
      'Potato crisps in sunflower oil'
    ],
    'labels': [
      'No colorings',
      'No preservatives',
      'Made in France',
      'Pommes de terres Françaises'
    ],
    'country': 'France',
    'ingredients': 'Pommes de terre, huile de tournesol, arôme naturel (contient PRODUITS LAITIERS), sel, poivre, FROMAGE de chèvre, herbes aromatiques.',
    'additifs': [],
    'allergenes': [
      'Gluten',
      'Mustard',
      'Peanuts'
    ],
    'nutriscore': 'D',
    'nutritional_value': {
      'energy': '515',
      'lipides': '32.1',
      'saturated_fat': '2.8',
      'glucides': '48.6',
      'sugar': '1.3',
      'fibres_alimentaires': '0',
      'proteines': '5.8',
      'salt': '1.3',
      'sodium': '0.513'
    },
    'image': 'https://static.openfoodfacts.org/images/products/333/697/150/5037/front_fr.5.400.jpg',
    'keywords': [
      'chips',
      'chèvre',
      'herbes',
    ],
  },

  // Produit 25

  {
    'id': '25',
    'product_name': 'Pain de mie Extra Moelleux',
    'brand': 'épi d\'or',
    'quantity': '500g',
    'packaging': [
      'sachet',
      'Plastique'
    ],
    'categories': [
      'Plant-based foods and beverages',
      'Plant-based foods',
      'Cereals and potatoes',
      'Breads',
      'Sliced breads'
    ],
    'labels': [],
    'country': 'France',
    'ingredients': 'Farine de blé 63%; eau; huile de colza; sucre; levure; dextrose; farine de fèves; sel; émulsifiants: lécithines (colza), E471, E472e, E482; gluten de blé; conservateur: E282; sirop de malt d\'orge; épaississant: gomme xanthane; agent de traitement de la farine: acide ascorbique.',
    'additifs': [
      'E322 - Lecithins',
      'E471 - Mono- and diglycerides of fatty acids',
      'E472e - Mono- and diacetyltartaric acid esters of mono- and diglycerides of fatty acids',
      'E482 - Calcium stearoyl-2-lactylate',
      'E282 - Calcium propionate',
      'E415 - Xanthan gum',
      'E300 - Ascorbic acid'
    ],
    'allergenes': [],
    'nutriscore': 'B',
    'nutritional_value': {
      'energy': '265',
      'lipides': '2.8',
      'saturated_fat': '0.4',
      'glucides': '50.2',
      'sugar': '5.2',
      'fibres_alimentaires': '3.1',
      'proteines': '8.2',
      'salt': '1.3',
      'sodium': '0.512'
    },
    'image': 'https://static.openfoodfacts.org/images/products/356/470/001/9320/front_fr.12.400.jpg',
    'keywords': [
      'pain',
      'mie',
      'pain de mie',
    ],
  },

];


export default products;
